...
@section('css')
    ...
    <style type="text/css">
        ...
        .ap {
            ...
            background-image: url(../vendor/quill-emoji/1e7b63404cd2fb8e6525b2fd4ee4d286.png);
    ...
    <div class="row">
            ...
            <div class="portlet light">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject bold font-blue uppercase"> Filtro </span>
                    </div>
                </div>
                <div class="portlet-body">
                    <div class="padding-tb-10 btn blue center-block" data-toggle="modal" data-target="#modal_clientes">
                        <h2 class="count_clientes margin-top-10">0</h2>
                    ...
                    <form id="form_filtro">
                        ...
                        <div class="form-group form-md-radios">
                            ...
                            <div class="md-radio-inline">
                                <div class="md-radio">
                                    <input type="radio" id="cliente_sexo_m" name="cliente[sexo]" class="md-radiobtn" value="M">
                                    <label for="cliente_sexo_m">
                                        <span></span>
                                        <span class="check"></span>
                                        <span class="box"></span> Masculino </label>
                                </div>
                                <div class="md-radio">
                                    <input type="radio" id="cliente_sexo_f" name="cliente[sexo]" class="md-radiobtn" value="F">
                                    <label for="cliente_sexo_f">
                                        <span></span>
                                        <span class="check"></span>
                        ...
                        <div class="form-group form-md-radios">
                            ...
                            <div class="md-radio-inline">
                                <div class="md-radio">
                                    <input type="radio" id="pet_sexo_m" name="pet[sexo]" class="md-radiobtn" value="M">
                                    <label for="pet_sexo_m">
                                        <span></span>
                                        <span class="check"></span>
                                        <span class="box"></span> Macho </label>
                                </div>
                                <div class="md-radio">
                                    <input type="radio" id="pet_sexo_f" name="pet[sexo]" class="md-radiobtn" value="F">
                                    <label for="pet_sexo_f">
                                        <span></span>
                                        <span class="check"></span>
                                        <span class="box"></span> Fêmea </label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group form-md-radios">
                            ...
                            <input type="text" name="pet[nome]" class="form-control">
                        </div>

                        <div type="submit" class="btn green-meadow form-control" id="search_button">Filtrar</div>
                        <div type="submit" class="btn yellow-crusta form-control" style="margin-top: 5px;" id="clear_button">Limpar</div>
        ...
        <div class="col-sm-8">
            <div class="portlet light">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-commenting font-blue"></i>
                        <span class="caption-subject bold font-blue uppercase"> Mensagem </span>
                    </div>
                </div>
                <div class="portlet-body">
                    <form action="{{ route('mobile.push.notifications.send') }}" id="form_push" class="form" method="POST">
                        {{ csrf_field() }}
                        <h2>Título</h2>
                        <div id="editor_title"></div>
                        <input type="hidden" name="titulo" id="input_titulo" required>

                        <h2>Corpo</h2>
                        <div id="editor_body"></div>
                        <input type="hidden" name="corpo" id="input_corpo" required>

                    </form>
                    <button class="btn green-meadow center-block margin-top-20" id="enviar_selecionados" style="min-width: 150px;">Enviar para os selecionados!</button>
                    <button class="btn blue center-block margin-top-20" id="enviar_teste">Enviar Teste...</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal" id="modal_clientes">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">Clientes selecionados</div>
...
@section('scripts')
    ...
    <script src="{{ url('/') }}/vendor/quill-emoji/quill-emoji.js"></script>
    <script>
        $(document).ready(function() {
            ...
            let editor_title = new Quill('#editor_title', {
                theme: 'snow',
                modules: {
            ...
            let editor_body = new Quill('#editor_body', {
                theme: 'snow',
                modules: {
                    toolbar: toolbarOptions,
                    "emoji-toolbar": true,
            ...
            function validateFormPush() {
                if ($('#input_titulo').val() === '' || $('#input_corpo').val() === '') {
                    return false;
                }
                return true;
            }
            ...
            $('#enviar_teste').click(function() {
                $('#input_titulo').val(editor_title.root.innerText.replace(/(\r\n|\n|\r)/gm, ""));
                $('#input_corpo').val(editor_body.root.innerText.replace(/(\r\n|\n|\r)/gm, ""));
                if (validateFormPush()) {
                    swal({
                        title: 'Enviando...',
                        ...
                        onOpen: () => {
                            ...
                            $.ajax({
                                method: "POST",
                                url: "{{ route('mobile.push.notifications.send_test') }}",
                                data: $('#form_push').serializeArray()
                            }).done(function ( res ) {
                                swal('Sucesso!', 'Teste enviado!', 'success');
                            });
                        }
                    });
                } else {
                    swal('Atenção!', 'Campos Obrigatórios!', 'warning');
                }
            });

            $('#enviar_selecionados').click(function() {
                $('#input_titulo').val(editor_title.root.innerText.replace(/(\r\n|\n|\r)/gm, ""));
                $('#input_corpo').val(editor_body.root.innerText.replace(/(\r\n|\n|\r)/gm, ""));
                if (validateFormPush()) {
                    swal({
                        ...
                        onOpen: () => {
                            ...
                            $.ajax({
                                method: "POST",
                                url: "{{ route('mobile.push.notifications.send') }}",
                                data: $('#form_push').serializeArray()
                            }).done(function ( res ) {
                                swal('Sucesso!', 'Notificações enviadas!', 'success');
                            });
                        }
                    });
                } else {
            ...
            function search_clientes() {
                $.ajax({
                    method: "POST",
                    url: "{{ route('mobile.push.notifications.search') }}",
                    data: $('#form_filtro').serializeArray()
                })
                .done(function( res ) {
                    console.log(res.length);
                    $('.count_clientes').text(res.length);
                    $('#modal_clientes ul#clientes_list li').remove();
                    $('#form_push .hidden_ids').remove();
                    $.each(res, function(i, cliente) {
                        $('#form_push').prepend('<input type="hidden" class="hidden_ids" name="id_clientes[]" value="' + cliente.id + '">');
                        $('#modal_clientes ul#clientes_list').append('<li><a href="../clientes/'+cliente.id+'/edit" target="_blank">#' + cliente.id + ' - ' + cliente.nome_cliente + '</a></li>');
                    });
                });
            }

            $('#form_filtro #search_button').click(function(e) {
                $('.count_clientes').text('. . .');
                search_clientes();
            });

            $('#form_filtro #clear_button').click(function(e) {
                $('.count_clientes').text('. . .');
                $('#form_filtro').trigger("reset");
                $('.count_clientes').text(0);
                $('#modal_clientes ul#clientes_list').html('');
            });
