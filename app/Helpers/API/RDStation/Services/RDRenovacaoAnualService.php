...
class RDRenovacaoAnualService {

    const IDENTIFIER = 'renovacao-anual-criada';
    ...
    public function process(Renovacao $renovacao, array $valores) {
        ...
        try {
            ...
            $logger->register(LogEvent::NOTIFY, LogPriority::LOW, "Acabamos de enviar um email de proposta de renovação com desconto para o cliente.\n$dadosJson", $renovacao->id);
        } catch (\Exception $exception) {
            $logger->register(LogEvent::ERROR, LogPriority::HIGH, "Não foi possível enviar o email de proposta da renovação com desconto.\n$dadosJson", $renovacao->id);
        };
    }
