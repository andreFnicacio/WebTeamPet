...
class Plans {
 ...
 //            "idplano"=> $info["id_plano"],
	...
	public function getCharge($idSacadoSac) {
		$curl = new Curl();
        ...
        $response = $curl->alternativeExecute();
        $curl->close();
        return $response;
	}

	public function getChargeById($idRecebimentoRecb) {
		$curl = new Curl();
	
        $curl->getDefaults(self::$urls["cobranca"] . "?id=" . $idRecebimentoRecb);
		...
		public function forceCreditCardProcessing($data) {
			$curl = new Curl();
			$curl->putDefaults(self::$urls["cobranca"]);
		...
		public function liquidateCharge($data) {
			$curl = new Curl();
			$curl->putDefaults(self::$urls["cobranca"] . '/liquidar');
			$curl->option(CURLOPT_POSTFIELDS, http_build_query($data));
